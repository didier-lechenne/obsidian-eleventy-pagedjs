...
export default class Layout extends Handler {
  ...
  handleClick(e) {
    ...
    if (element && this.isInModularGrid(element)) {
      ...
      // Sélection normale SANS Shift pour redimensionnement/déplacement
  ...
  handleMouseEnter(e) {
    if (this.state.isResizing || !document.body.classList.contains("layout"))
      return;
    ...
    if (
      element &&
      this.isInModularGrid(element) &&
      element !== this.state.hoveredElement
    ) {
      // Pour les images : survol actif uniquement avec Shift
      if (element.dataset.grid === "image") {
        if (e.shiftKey) {
          this.setHoveredElement(element);
        }
      } else {
        // Pour les autres éléments : survol normal
        this.setHoveredElement(element);
      }
    }
  }
  ...
  handleMouseLeave(e) {
    ...
    // Ne pas nettoyer les éléments sélectionnés
  ...
  handleMouseOver(e) {
    ...
    // Pour les images : activation uniquement avec Shift
    if (element.dataset.grid === "image") {
      ...
      // Pour les autres éléments : activation normale au survol
  ...
  setHoveredElement(element) {
    ...
    // Pour les éléments non-image, ajoute le bouton de déplacement
    if (element.dataset.grid !== "image") {
  ...
  // Sélectionne un élément et met à jour l'interface
  selectElement(element) {
    ...
    // Met à jour l'interface utilisateur (layout-form)
  ...
  handleMouseDown(e) {
    ...
    // Drag d'image : conserve le comportement avec Shift
    if (e.shiftKey) {
    ...
    // Détection du mode de redimensionnement SANS Shift
  ...
  handleImageDragMove(e) {
    if (!this.state.isDragging || !this.state.currentImage || !e.shiftKey)
      return;
  ...
  handleKeyDown(e) {
    if (e.key === "Shift" && !this.state.isShiftPressed) {
      ...
      // Cache les boutons de déplacement quand Shift est pressé
      this.toggleMoveButtons(false);
      
      // Active le mode image si on survole une image
      const hoveredElement = document.querySelector('.modularGrid [data-grid]:hover');
      if (hoveredElement && hoveredElement.dataset.grid === "image") {
    ...
    // Déplacement d'image avec les flèches (conserve Shift)
    const moves = {
      ArrowUp: [0, -2],
      ArrowDown: [0, 2],
      ArrowLeft: [-2, 0],
    ...
    // Zoom d'image avec +/- (conserve Shift)
    const zoomActions = {
      Equal: 1.005,
      Minus: 0.995,
      NumpadAdd: 1.005,
  ...
  handleKeyUp(e) {
    if (e.key === "Shift" && this.state.isShiftPressed) {
      ...
      // Réaffiche les boutons de déplacement
      this.toggleMoveButtons(true);
      
      // Désactive le mode survol pour les images si plus de Shift
      if (this.state.hoveredElement && 
          this.state.hoveredElement.dataset.grid === "image" && 
          !this.state.hoveredElement.matches(":hover")) {
        this.clearHover();
      }
    }
  }
