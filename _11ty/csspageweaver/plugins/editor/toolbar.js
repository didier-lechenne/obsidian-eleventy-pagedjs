...
import { LetterSpacingExtension } from './letter-spacing-extension.js';
...
class FrenchExtension {
  ...
  resetTransformations() {
    ...
    const otherAddedElements = element.querySelectorAll(
      ".editor-add:not(.i_space)"
    );
    ...
    // Ajouter dans resetTransformations() :
    const letterSpacingSpans = element.querySelectorAll('span.letter-spacing.editor-add');
...
export class Toolbar {
    ...
    this.turndown = new window.TurndownService({
      ...
      rules: {
        ...
        removeSpaceSpans: {
          filter: function (node) {
            return (
              node.nodeName === "SPAN" && node.className.includes("i_space")
            );
          },
          replacement: function (content) {
            return content;
        ...
        footnoteCall: {
          ...
          replacement: function (content, node) {
            ...
            if (footnoteContent) {
              ...
              const noteMarkdown =
                utilsExt.convertNoteContentToMarkdown(footnoteContent);
              return `^[${noteMarkdown}]`;
            }
  ...
  bindEvents() {
    ...
    // Remplacer l'event listener existant par :
    this.element.addEventListener('input', (e) => {
      console.log('Input detected on:', e.target);
      if (e.target.matches('[data-command="letter-spacing-input"]')) {
        console.log('Letter spacing input matched');
        const extension = this.extensions.find(ext => ext instanceof LetterSpacingExtension);
        if (extension) {
          extension.handleInputChange(e.target);
        }
      }
    });
