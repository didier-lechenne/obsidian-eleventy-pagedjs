...
export const ACTIONS_REGISTRY = {
  ...
  "letter-spacing": {
    ...
    execute: (editor) => {
      ...
      if (!selection?.isValid) return; // Sortir si pas de sélection
      ...
      if (input.classList.contains("ls-hide")) {
        // Montrer l'input et créer le span
      ...
      while (current && current !== document.body) {
        if (
          ...
          if (button && input) {
            button.classList.add("active");
            input.classList.remove("ls-hide");
            input.value = current.style.getPropertyValue("--ls") || "0";
            input.setAttribute(
              "data-target-span",
              current.getAttribute("data-ls-id")
            );
          }
